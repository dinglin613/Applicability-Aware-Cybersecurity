...
| `N=|\mathcal{D}|` | Number of cyber-relevant devices. |
| `d_i` | Device indexed by `i`. |
| `\mathcal{F}` | Set of operational functions. |
| `M=|\mathcal{F}|` | Number of operational functions; used in complexity expressions. |
...
| `u_i(t)\in\{0,1\}` | Patch state of device `d_i`; `0` unpatched, `1` patched. |
| `y_i(t)\in\{0,1\}` | Online state of device `d_i`; `0` during patching, `1` online. |
